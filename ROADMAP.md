...
- [ ] **📝 继续完善 梨记 (lily-note)**
  - [ ] 探索进一步的优化与扩展方向（如阅读量统计、草稿箱增强、或更丰富的渲染能力，目前方向待定）。

---

## 🏆 已完成的里程碑 (Milestones & Changelog)

### 🔴 v0.3.0 梨记上线：纯文件驱动的轻量级个人笔记模块 (当前版本)
- [x] **📝 新增 note 笔记页面与后台管理 (Lily-Note)**
  - [x] 引入 `notes/` 存储目录，完全由纯 `.md` 文件驱动，无需数据库，提取 TOML 元数据（标题、时间、摘要、标签等）。
  - [x] 实现笔记列表页与详情页路由，内置 `pulldown-cmark` 渲染引擎，防范 XSS 攻击。
  - [x] 在前端实现零请求开销的原生 JavaScript 全文检索功能（支持按标题、摘要和标签瞬时过滤）。
  - [x] 在管理后台（/admin）完美集成笔记在线管理面板，支持可视化的新增、编辑、删除操作，并在保存时以 100% 异步非阻塞 (`tokio::fs`) 方式安全落盘。

### 🔴 v0.2.9 / v0.2.8 模板解耦、更新日志与安全升级版
